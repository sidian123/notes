...
>
>![在这里插入图片描述](.Spring Boot/20190606173328676.png)
>如上配置后，每次切换程序，会自动更新静态文件，点击`update`按钮，触发devtools的restart。`update`按钮的快捷键：`Ctrl+F10`
...
通过属性`logging.config`属性指定，根据你的日记系统，不同的文件会被加载：
