...
>或者kill [-signal] pid...
>kill -l 
>`-s`指定要发送的信号，默认term。signal可以是信号名或对应的数值。完整信号名前3个字符“SIG”可以省略。
>`-l`打印所有信号及对应数值。
...
进程可以捕获（catch）信号，然后执行自己的处理，如果没有捕获则会执行默认处理。如ping捕获int信号（ctrl+c），打印最终结果并结束程序。但是`kill`和`stop`信号不能够被捕获，因此必定执行默认行为。
...
* 系统cron任务：以root用户执行，常用于执行系统维护。需在`/etc/crontab`中配置。
	...
	>`/etc/cron.d/`呢？不太懂
* 用户cron任务：普通用户创建的任务，配置位于`/var/spool/cron`目录下。建议使用命令`crontab`创建任务，而不是直接操作配置文件。
