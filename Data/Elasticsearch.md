...
可以被索引的基本数据单位，也是全文搜索中被搜索的对象，可以对应一个网页，一篇txt文档或者一个商品。 相当于关系型数据库中的表中的一行记录。
...
* 全文索引
  ...
  * 目的
    ...
    直接建立从**词汇**到**文档**的映射(倒排索引)

  * 构建过程

    有三篇文档

    ![image-20200704134932693](.Elasticsearch/image-20200704134932693.png)

    对文章进行分词

    ![image-20200704135031645](.Elasticsearch/image-20200704135031645.png)

    将分词按照一定顺序(如词典的顺序)排序, 构建词汇到文档Id的映射

    ![image-20200704135129619](.Elasticsearch/image-20200704135129619.png)

  * 查询过程

    因为分词索引是有序的, 所以查找词汇的时间复杂度为O(log2n). 接着分词与文档间是键值映射, 所以时间复杂度为O(1). 因此总的时间复杂度为O(log2n)

  > 参考https://zhuanlan.zhihu.com/p/94181307
