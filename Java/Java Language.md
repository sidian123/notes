...
* **变量**: 存储数据的地方. 每个变量都对应一个数据类型, 数据类型决定了变量可以存储何种类型的数据, 以及可对该数据进行何种操作.

  > 这里的变量主要指存在栈区的变量, 对象主要指存在堆中的对象.
  >
  > 同时本文可能不会特别区分两者关系, 会混淆的使用这概念.
...
* **类和对象**: 
  ...
  * 类也是一种数据类型, 它的字段声明定义了对象能够存储的数据, 它的方法声明定义了对象能够拥有的操作.

  * 类由字段声明, 初始化代码块声明,构造函数声明, 方法声明组成.

* **接口**: 接口与类相比, 接口普通方法不能有实现, 也即接口只是定义了行为, 需要实现类去实现.
* **Packages**: 就是含有Java代码的目录, 用于分组管理代码.
...
- Non-static fields: 属于对象的字段, 每个对象都有一份, 只能被对象访问.
- Static fields: 属于类的字段, 每个类只有一份, 可以通过类和对象访问到.
- Local variables: 方法内的局部变量
- Parameters: 方法参数

从变量是否指向对象可分为

- 基本变量(primitive variable): 如`int`,`long`
- 引用变量(reference variable)或对象引用(Objecgt references): 如`Integer`,`Long`
...
| ------------- | ------------------------------------------------------------ | ----------- |
| `boolean`     | A binary value of either `true` or `false`                   | `Boolean`   |
| `byte`        | 8 bit signed value, values from -128 to 127                  | `Byte`      |
| `short`       | 16 bit signed value, values from <br />-32.768 to 32.767     | `Short`     |
| `char`        | 16 bit Unicode character                                     | `Character` |
| `int`         | 32 bit signed value, values from <br />-2.147.483.648 to 2.147.483.647 | `Integer`   |
| `long`        | 64 bit signed value, values from <br />-9.223.372.036.854.775.808 to 9.223.372.036.854.775.808 | `Long`      |
| `float`       | 32 bit floating point value                                  | `Float`     |
...
基本类型的包装类是**不可修改的**, 因为除了声明时的初始化, 包装类并没有提供修改值的函数. 只能new一个新对象, 赋值给变量.
